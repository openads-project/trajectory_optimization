...
RCLCPP_COMPONENTS_REGISTER_NODE(trajectory_optimization::TrajectoryOptimizationRWSNode)
...
std::vector<double> TrajectoryOptimizationRWSNode::getBiLevelX0(const perception_msgs::msg::EgoData& ego_data) {
  ...
  try {
    tf_trajectory = tf2_buffer_->transform(latest_valid_trajectory_, vehicle_frame_id_, tf2_ros::fromMsg(ego_data.header.stamp),
...
void TrajectoryOptimizationRWSNode::convertToTrajectoryMsg(trajectory_planning_msgs::msg::Trajectory& trajectory) {
  for (int i = 0; i <= n_shots_; ++i) {
    ...
    trajectory_planning_msgs::trajectory_access::setBeta(
        trajectory, xtraj_[i * *nlp_dims_->nx + 6], xtraj_[i * *nlp_dims_->nx + 7], distance_front_axle_, distance_rear_axle_, i);
  }
}
...
double TrajectoryOptimizationRWSNode::projectVectorAonV(const geometry_msgs::msg::Vector3& a,
