...
void TrajectoryOptimizationAckermannNode::setTrajectoryType(trajectory_planning_msgs::msg::Trajectory& trajectory) {
    // DRIVEABLE trajectory type
...
std::vector<double> TrajectoryOptimizationAckermannNode::getBiLevelX0(const perception_msgs::msg::EgoData& ego_data) {
    ...
    std::vector<double> TIME, V, Y, A, THETA, DELTA_FRONT;
    ...
    double v_tgt, y_tgt, a_tgt, theta_tgt, delta_front_tgt;
    double des_time =
...
void TrajectoryOptimizationAckermannNode::printStateInfo(const std::vector<double>& state) {
    RCLCPP_DEBUG(this->get_logger(), "State: x: %f, y: %f, s: %f v: %f, a: %f, theta: %f, delta: %f ", state[0],
