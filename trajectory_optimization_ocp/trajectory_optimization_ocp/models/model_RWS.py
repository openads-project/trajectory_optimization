...
def set_model(ocp, config):
    ...
    beta = atan(
        (L_r / (L_f + L_r)) * stable_tan(state[STATE_INDEX_DELTA_F])
        + (L_f / (L_f + L_r)) * stable_tan(state[STATE_INDEX_DELTA_R])
    )
    ...
    f_psi_dot = (
        state[STATE_INDEX_V_T]
        * cos(beta)
        * (stable_tan(state[STATE_INDEX_DELTA_F]) - stable_tan(state[STATE_INDEX_DELTA_R]))
        / (L_f + L_r)
    )
    ...
    p_constraint_activation = MX.sym("constraint_activation", np.prod(config["p_constraint_activation_shape"]))
    p_objects = MX.sym("objects", np.prod(config["p_objects_shape"]))
    params = vertcat(p_dynamic_weight, p_constraint_activation, p_objects)
    model.p = params
    ...
    )  # (thw, d_min_object_long, d_min_object_lat, d_min_boundary_lat) -> 4
    p_ref_path = MX.sym(
