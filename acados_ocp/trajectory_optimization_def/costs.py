...
def set_costs(ocp, parameter):    
    ...
    # y_ref = y1 + lmd * (y2-y1)
    # v_ref = v1 + lmd * (v2-v1)
    x_ref = ref_path[idx_min, 1]
    y_ref = ref_path[idx_min, 2]
    v_ref = ref_path[idx_min, 3]
    ...
    cost.yref = np.array([x_ref, y_ref, 0])
    cost.yref_0 = np.array([x_ref, y_ref, 0])
    cost.yref_e = np.array([x_ref, y_ref, 0])
